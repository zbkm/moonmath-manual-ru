...
SAGE_ROOT/venv/share/doc/sagetex.""")
  ...
  def commandline(self, counter, s, globals, locals, text_output):
      ...
      for i in range(len(splitup)):
          ...
          code = strip_common_leading_spaces(orig_input.strip('\n'))
          boxname = '@sagecmdline{}-code{}'.format(counter, i)
          to_tmp = [r'\begin{SaveVerbatim}{' + boxname + '}', code, r'\end{SaveVerbatim}']
          self.souttmp.write('\n'.join(to_tmp) + '\n')
          tex_strs.append(r'\UseVerbatim{' + boxname + '}')
          try:
              result = eval(preparse(splitup[i][2]), globals, locals)
              if text_output:
                  out = str(result)
                  boxname = '@sagecmdline{}-out{}'.format(counter, i)
                  to_tmp = [r'\begin{SaveVerbatim}{' + boxname + '}', out, r'\end{SaveVerbatim}']
                  self.souttmp.write('\n'.join(to_tmp) + '\n')
                  tex_strs.append(r'\UseVerbatim{' + boxname + '}')
              else:
